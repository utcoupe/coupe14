...
class CommunicationGlobale():
		...
		for order in self.orders:
			...
			for i,typeToGet in enumerate(self.ordersRetour[order]):
				if typeToGet == 'int':
					...
					self.__loger.error("ERREUR: Parseur: le parseur a trouvé un type non supporté")
	...
	def gestion(self):
		while THREAD_ACTIF:
							...
							if len(indiceARenvoyer) > 0:
									...
									if self.nbRenvoiImmediat[address] != 0:
										for i in range(self.nbRenvoiImmediat[address]):
											if i < len(indiceARenvoyer):
												self.__loger.warning("Renvoie immediat de l'ordre: ", self.orders[self.ordreLog[address][indiceARenvoyer[i]][0]], "d'idd ", indiceARenvoyer[i], "au robot ", self.address[address])
												...
												self.__loger.critical("cas impossible 42")
								...
								if (date - self.nbUnconfirmedPacket[address][1]) > TIMEOUT and self.nbUnconfirmedPacket[address][1] != -1:
									for indice in indiceARenvoyer:
										...
										self.__loger.warning("Renvoie après TIMEOUT de l'ordre: ", self.orders[self.ordreLog[address][indice][0]], "d'idd ", indice, "au robot ", self.address[address], "binaire :", self.ordreLog[address][indice])
						...
						if isinstance(address, (int)):
							if self.arduinoIdReady[address] != False:
								if (date - self.lastConfirmationDate[address]) > OFF_LIGNE_TIMEOUT and (date - self.arduinoIdReady[address]) > OFF_LIGNE_TIMEOUT:#le système est considere comme hors ligne
									self.__loger.error("L'arduino", self.address[address], "va être reset car elle a depasser le TIMEOUT")
			...
			if waitBeforeNextExec < 1:
				self.__loger.warning("La boucle de pool de communication n'est pas assez rapide ", waitBeforeNextExec)
			else:
	...
	def removeOrdersInFile(self, address):# Warning, only on reset !
		...
		for packet in self.ordersToSend:
			if packet[0] != address:
				...
				self.__loger.critical("drop de l'ordre",self.orders[ packet[1]], " par l'arduino", self.address[ packet[0]], "suite à un reset")
	...
	def acceptConfirmeResetId(self, address):#accepte la confirmation de reset d'un arduino
		self.__loger.info("L'arduino ", self.address[address], " a accepte le reset")
		self.removeOrdersInFile(address)
		self.lastConfirmationDate[address] = -1
		self.nbUnconfirmedPacket[address] = (0, -1)
		self.lastSendDate[address] = -1
	...
	def confirmeResetId(self, address):#renvoie une confirmation de reset
		self.__loger.info("Reponse à la demande de confirmation de reset de l'arduino ", self.address[address])
		self.removeOrdersInFile(address)
		self.lastConfirmationDate[address] = -1
		self.nbUnconfirmedPacket[address] = (0, -1)
		self.lastSendDate[address] = -1
	...
	def extractData(self, rawInput):
		...
		if packetAddress > 96:# l'arduino confirme le reset
			if packetAddress-96 in self.address:
				...
				self.__loger.warning("corrupted address on reset confirme from arduino, adress", packetAddress-96)
				return -1
			...
			if packetAddress-64 in self.address:
				...
				self.__loger.warning("corrupted address on reset request from arduino, address", packetAddress-64)
				return -1
			...
			if packetAddress > 0 and packetAddress < (self.nbAddress+1) and packetId >= 0 and packetId < 64:
				...
				if order != -1:
					if len(rawInput[2:-1])*7//8 == self.returnSize[ order ]:
						...
						self.__loger.warning("Le paquet est mal formé, l'address ou l'id est invalide debug_A")
						return -1
				else:
					self.__loger.error("On essaye de lire, l'id", packetId, "en provenance de l'arduino", self.address[packetAddress], "mais il n'est existe pas de trace dans le log (un vieux paquet qui trainait sur un client avant la nouvelle init ?)")
					return -1
			elif packetAddress > 0 and packetAddress < (self.nbAddress+1) and packetId > 63:
				self.__loger.warning("L'arduino", self.address[packetAddress], "nous indique avoir mal reçu un message, message d'erreur ", packetId)
				if self.nbNextRenvoiImmediat[packetAddress] != 0:
					self.nbRenvoiImmediat[packetAddress] += 1
				return -1
			else:
				self.__loger.warning("Le paquet est mal formé, l'address ou l'id est invalide")
				return -1
		else:
			self.__loger.warning("Le paquet n'est pas un reset et ne fait même pas 3 octet, des données ont probablement été perdue, paquet droppé")
			return -1
			...
			if idd in unconfirmedIds:
				...
				if idd == self.getNextConfirmeId(address):
					if self.ordreLog[address][idd][0] != self.orders['PINGPING_AUTO']:
						self.__loger.info("Success: l'arduino", self.address[address]," a bien recu l'ordre ", self.orders[self.ordreLog[address][idd][0]], " d'id: ", idd)
					...
					while lastIdToAccept != idd and returnMissed == False:
						...
						if i > MAX_UNCONFIRMED_PACKET:
							self.__loger.critical("ERREUR CODE: ce cas ne devrait pas arriver")
						i +=1

					if lastIdToAccept != self.lastIdConfirm[address]:
						if returnMissed == True:
							self.__loger.info("Success: l'arduino", self.address[address]," a bien recu les ordres jusque", idd, "mais il manque au moins un retour (avec argument) donc on ne confirme que", self.orders[self.ordreLog[address][lastIdToAccept][0]], " d'id: ", lastIdToAccept)
							self.nbTransmitedPaquets += 1
				...
				if idd == self.getNextConfirmeId(address) or lastIdToAccept != self.lastIdConfirm[address]:
					...
					for returnType in self.ordersRetour[self.ordreLog[address][idd][0]]:
						if returnType == 'int':
							...
							self.__loger.error("Parseur: le parseur a trouvé un type non supporté")
				...
				self.__loger.warning("l'arduino", self.address[address], "a accepte le paquet", idd, "alors que les paquets a confirmer sont ", self.getAllUnknowledgeId(address), " sauf si on a louppé une réponse avec arguments")
	...
	def applyProtocole(self, address, idd, order, data):
		...
		for i,typeToGet in enumerate(self.ordersArguments[order]):
			if typeToGet == 'int':
				...
				self.__loger.error("ERREUR: Parseur: le parseur serial_defines a trouvé un type non supporté")
	...
	def sendOrders(self):
		...
		for address in self.address:
			if isinstance(address, (int)):
				while self.nbNextRenvoiImmediat[address] > 0 and self.nbUnconfirmedPacket[address][0] < MAX_UNCONFIRMED_PACKET:
					self.__loger.warning("procédure de renvoi après un renvoi immediat sur l'arduino", self.address[address], "du paquets d'id", self.getNextIdOfId(self.lastIdSend[address]))
		...
		for packet in self.ordersToSend:#packet contient(address, ordre, *argument)
			...
			if self.nbUnconfirmedPacket[packet[0]][0] < MAX_UNCONFIRMED_PACKET:
				...
				self.__loger.info("Envoi normal a l'arduino", self.address[address], "de l'ordre", self.orders[order], "d'id", idd)
				self.sendMessage(address, chaineTemp)
			else:
				remainOrdersToSend.appendleft(packet)
		...
		if len(remainOrdersToSend) == 0 and not self.empty_fifo:
			self.empty_fifo = True
			self.__loger.info("Fin de transmission de la file, (t = "+str(int(time.time()*1000)-self.timeStartProcessing)+"ms),nombre de paquets reçu", self.nbTransmitedPaquets," nombre de paquets perdu", self.nbTimeoutPaquets)
	...
	def checkAddress(self, address):
		...
		if address in self.address:
			...
			if self.arduinoIdReady[address] != False:
				...
				self.__loger.error("L'arduino", self.address[address], " n'est pas prête.")
				return -1
		else:
			self.__loger.error("L'address: ", address, " est invalide.")
			return -1
	...
	def checkOrder(self, order):
		...
		if order in self.orders:
			...
			self.__loger.error("L'ordre: ", order, " est invalide.")
			return -1
		...
		if order in self.argumentSize:#verification
			...
			for argumentType in self.ordersArguments[order]:
				if argumentType == 'int':
					...
					self.__loger.error("type parse inconnu")
			if somme != sizeExpected:
				self.__loger.error("la constante de taille de l'ordre ", order, " ne correspond pas aux types indiqués attendu ", sizeExpected, " calculee ", somme)
		else:
			self.__loger.error("ERREUR l'ordre ", order, "n'a pas été trouvé dans serial_defines.c")
			...
			for i, argumentType in enumerate(self.ordersArguments[order]):
				if argumentType == 'int':
					if not isinstance(arguments[i], (int)):
						self.__loger.error("L'argument ", i, " de l'ordre ", order, " n'est pas du bon type, attendu (int)")
						return -1
				elif argumentType == 'long':
					if not isinstance(arguments[i], (long)):
						self.__loger.error("L'argument ", i, " de l'ordre ", order, " n'est pas du bon type, attendu (long)")
						return -1
				elif argumentType == 'float':
					if not isinstance(arguments[i], (float)):
						self.__loger.error("L'argument ", i, " de l'ordre ", order, " n'est pas du bon type, attendu (float)")
						return -1
				else:
					self.__loger.error("l'argument parsé dans serial_define est de type inconnu")
					return -1
					
		else:
			self.__loger.error("l'order", order, "attend", len(self.ordersArguments[order]), "arguments, mais a recu:", len(arguments), "arguemnts")
			return -1

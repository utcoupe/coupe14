...
class CommunicationGlobale():
	...
	def gestion(self):
		while THREAD_ACTIF:
							...
							if len(indiceARenvoyer) > 0:
									...
									if self.nbRenvoiImmediat[address] != 0:
										for i in range(self.nbRenvoiImmediat[address]):
											if i < len(indiceARenvoyer):
												self.__logger.warning("Renvoie immediat de l'ordre: ", self.orders[self.ordreLog[address][indiceARenvoyer[i]][0]], "d'idd ", indiceARenvoyer[i], "au robot ", self.address[address])
								...
								if (date - self.nbUnconfirmedPacket[address][1]) > TIMEOUT and self.nbUnconfirmedPacket[address][1] != -1:
									for indice in indiceARenvoyer:
										...
										self.__logger.warning("Renvoie après TIMEOUT de l'ordre: ", self.orders[self.ordreLog[address][indice][0]], "d'idd ", indice, "au robot ", self.address[address], "binaire :", self.ordreLog[address][indice])
						...
						if isinstance(address, (int)):
							if self.arduinoIdReady[address] != False:
								if (date - self.lastConfirmationDate[address]) > OFF_LIGNE_TIMEOUT and (date - self.arduinoIdReady[address]) > OFF_LIGNE_TIMEOUT:#le système est considere comme hors ligne
									self.__logger.error("L'arduino", self.address[address], "va être reset car elle a depasser le TIMEOUT")
			...
			if waitBeforeNextExec < 1:
				self.__logger.warning("La boucle de pool de communication n'est pas assez rapide ", waitBeforeNextExec)
			else:
	...
	def removeOrdersInFile(self, address):# Warning, only on reset !
		...
		for packet in self.ordersToSend:
			if packet[0] != address:
				...
				self.__logger.critical("drop de l'ordre",self.orders[ packet[1]], " par l'arduino", self.address[ packet[0]], "suite à un reset")
	...
	def acceptConfirmeResetId(self, address):#accepte la confirmation de reset d'un arduino
		self.__logger.info("L'arduino ", self.address[address], " a accepte le reset")
		self.removeOrdersInFile(address)
		self.lastConfirmationDate[address] = -1
		self.nbUnconfirmedPacket[address] = (0, -1)
		self.lastSendDate[address] = -1
	...
	def confirmeResetId(self, address):#renvoie une confirmation de reset
		self.__logger.info("Reponse à la demande de confirmation de reset de l'arduino ", self.address[address])
		self.removeOrdersInFile(address)
		self.lastConfirmationDate[address] = -1
		self.nbUnconfirmedPacket[address] = (0, -1)
		self.lastSendDate[address] = -1
	...
	def extractData(self, rawInput):
		...
		if packetAddress > 96:# l'arduino confirme le reset
			if packetAddress-96 in self.address:
				...
				self.__logger.warning("corrupted address on reset confirme from arduino, adress", packetAddress-96)
				return -1
			...
			if packetAddress-64 in self.address:
				...
				self.__logger.warning("corrupted address on reset request from arduino, address", packetAddress-64)
				return -1
			...
			if packetAddress > 0 and packetAddress < (self.nbAddress+1) and packetId >= 0 and packetId < 64:
				...
				if order != -1:
					...
					self.__logger.error("On essaye de lire, l'id", packetId, "en provenance de l'arduino", self.address[packetAddress], "mais il n'est existe pas de trace dans le log (un vieux paquet qui trainait sur un client avant la nouvelle init ?)")
					return -1
			elif packetAddress > 0 and packetAddress < (self.nbAddress+1) and packetId > 63:
				self.__logger.warning("L'arduino", self.address[packetAddress], "nous indique avoir mal reçu un message, message d'erreur ", packetId)
			...
			if idd in unconfirmedIds:
				...
				if idd == self.getNextConfirmeId(address):
					if self.ordreLog[address][idd][0] != self.orders['PINGPING_AUTO']:
						self.__logger.info("Success: l'arduino", self.address[address]," a bien recu l'ordre ", self.orders[self.ordreLog[address][idd][0]], " d'id: ", idd)
					...
					if lastIdToAccept != self.lastIdConfirm[address]:
						if returnMissed == True:
							self.__logger.info("Success: l'arduino", self.address[address]," a bien recu les ordres jusque", idd, "mais il manque au moins un retour (avec argument) donc on ne confirme que", self.orders[self.ordreLog[address][lastIdToAccept][0]], " d'id: ", lastIdToAccept)
							self.nbTransmitedPaquets += 1
				...
				self.__logger.warning("l'arduino", self.address[address], "a accepte le paquet", idd, "alors que les paquets a confirmer sont ", self.getAllUnknowledgeId(address), " sauf si on a louppé une réponse avec arguments")
	...
	def sendOrders(self):
		...
		for address in self.address:
			if isinstance(address, (int)):
				while self.nbNextRenvoiImmediat[address] > 0 and self.nbUnconfirmedPacket[address][0] < MAX_UNCONFIRMED_PACKET:
					self.__logger.warning("procédure de renvoi après un renvoi immediat sur l'arduino", self.address[address], "du paquets d'id", self.getNextIdOfId(self.lastIdSend[address]))
		...
		for packet in self.ordersToSend:#packet contient(address, ordre, *argument)
			...
			if self.nbUnconfirmedPacket[packet[0]][0] < MAX_UNCONFIRMED_PACKET:
				...
				self.__logger.info("Envoi normal a l'arduino", self.address[address], "de l'ordre", self.orders[order], "d'id", idd)
				self.sendMessage(address, chaineTemp)
			else:
				remainOrdersToSend.appendleft(packet)
		...
		if len(remainOrdersToSend) == 0 and not self.empty_fifo:
			self.empty_fifo = True
			self.__logger.info("Fin de transmission de la file, (t = "+str(int(time.time()*1000)-self.timeStartProcessing)+"ms),nombre de paquets reçu", self.nbTransmitedPaquets," nombre de paquets perdu", self.nbTimeoutPaquets)
	...
	def checkAddress(self, address):
		...
		if address in self.address:
			...
			if self.arduinoIdReady[address] != False:
				...
				self.__logger.error("L'arduino", self.address[address], " n'est pas prête.")
				return -1
		else:
			self.__logger.error("L'address: ", address, " est invalide.")
			return -1
	...
	def checkOrder(self, order):
		...
		if order in self.orders:
			...
			self.__logger.error("L'ordre: ", order, " est invalide.")
			return -1
	...
	def checkParsedOrderSize(self, order):
		...
		if order in self.argumentSize:#verification
			...
			if somme != sizeExpected:
				self.__logger.error("la constante de taille de l'ordre ", order, " ne correspond pas aux types indiqués attendu ", sizeExpected, " calculee ", somme)
		else:
			self.__logger.error("ERREUR l'ordre ", order, "n'a pas été trouvé dans serial_defines.c")
	...
	def checkOrderArgument(self, order, *arguments):
		...
		if len(arguments) == len(self.ordersArguments[order]):
			for i, argumentType in enumerate(self.ordersArguments[order]):
				if argumentType == 'int':
					if not isinstance(arguments[i], (int)):
						self.__logger.error("L'argument ", i, " de l'ordre ", order, " n'est pas du bon type, attendu (int)")
						return -1
				elif argumentType == 'long':
					if not isinstance(arguments[i], (long)):
						self.__logger.error("L'argument ", i, " de l'ordre ", order, " n'est pas du bon type, attendu (long)")
						return -1
				elif argumentType == 'float':
					if not isinstance(arguments[i], (float)):
						self.__logger.error("L'argument ", i, " de l'ordre ", order, " n'est pas du bon type, attendu (float)")
						return -1
				else:
			...
			self.__logger.error("l'order", order, "attend", len(self.ordersArguments[order]), "arguments, mais a recu:", len(arguments), "arguemnts")
			return -1

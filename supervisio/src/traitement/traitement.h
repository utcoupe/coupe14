...
class Visio {
	public:
		...
		//FILE
		bool loadPerspectiveMatrix(string filename);
		void loadParams(string filename);
		bool loadCameraMatrix(string filename);
		void savePerspectiveMatrix(string filename);
		void saveParams(string filename);
		void saveCameraMatrix(string filename);
		//SETTER
	...
	private:
		...
		void addTriangle(const Point2f& point_real, const vector<Point2f>& contour_real, vector<Triangle>& triangles, double size);
